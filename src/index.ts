...
import { ProductPerformanceRecord, SchemaField } from './types.js';
...
function preprocessValue(value: any, dtype: string, notes: string): any {
  // 处理空值和N/A
  ...
  if (dtype === 'object' && notes.includes('NaT')) {
    // 特殊处理stockout_date
    if (strValue.toLowerCase() === 'n/a') return null;
    try {
...
// 自动扫描test目录下的所有报表文件
function getTestDirectoryFiles(): string[] {
    ...
    for (const filePath of filePaths) {
      ...
      for (let i = 0; i < raw_rows.length; i++) {
        ...
        try {
          ...
          if (result.inserted) inserted_rows++; else updated_rows++;
...
server.tool(
  ...
  async () => {
    ...
    const missing_values_summary: Record<string, number> = {};
    for (const field of productPerformanceSchema.fields) {
    ...
    const key_numeric_stats: Record<string, any> = {};
